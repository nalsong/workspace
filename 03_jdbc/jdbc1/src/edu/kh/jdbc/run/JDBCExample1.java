...
public class JDBCExample1 {
	public static void main(String[] args) {
		...
		// JDBC : Java에서 DB에 연결(접근)할 수 있게 해주는 코드 
		// 		  (class, interface)를 제공해주는 것
		//			-> Java Progamming API
		//			-> java.sql 패키지에서 제공
		 ...
		 * 2. DBMS의 종류(Oracle)
		 * 3. DBMS 회사에서 제공하는 Java 애플리케이션과 DB연결을 위한 라이브러리
		...
		// DB 연결 정보를 담는 객체
		// --> Connection을 상속받아 구현한 자식 객체가 대입될 예정
		
		// DBMS이름, 타입, IP, PORT, 계정명, 비밀번호 저장
		// -> DBeaver의 계정 접속 방법 추가와 유사함
		
		// 왜 Connection은 인터페이스 일까?
		// -> DBMS마다 연결 정보, 방법이 다르기 때문에 상속을 받아 공통된 부분을 
		//	  DBMS마다 알맞게 구현하고 구현된 객체를 Java에서 참조해서 사용

		Statement stmt = null;
		// The object used for executing a static SQL statementand returning the results it produces
		// sql을 DB에 전달해 수행하고 결과(Result set 또는 DML 성공 행의 개수)를 반환 받는데 사용되는 객체
		
		ResultSet rs = null;
		// SELECT 질의 성공 시 반환되는 객체
		// 조회결과의 집합을 나타냄
		// -> 테이블 모양으로 반환됨
		// -> ResultSet 객체를 이용해서 1행씩 접금
		// -> 더 이상 접근할 행이 없을 경우 false반환
		
		
		try {
			// 2단계 : 1단계에서 선언한 참조변수에 알맞은 객체 대입
			
			// 1. DB연결에 필요한 Oracle JDBC Driver 객체를 메모리에 로드하기
			// -> OracleDriver.class를 객체로 만들어두기
			
			Class.forName("oracle.jdbc.driver.OracleDriver");
			// -> ()안에 작성된 경로에 존재하는 클래스를 이용해 객체를 생성
			// -> 메모리에 객체가 생성되고 Connection을 이용한 DB연결 시 이를 자동으로 참조한다.
			
			// ClassNotFoundException 발생가능성이 있음
			// -> 오타 또는 라이브러리 추가 안 했을 시 발생
			...
			String ip = "localhost";
			// localhost == 127.0.0.1 (loop back ip == 본인 컴퓨터 주소)
			// 115.90.212.22 (강의장 DB서버 이용자)
			
			String port = ":1521"; // 컴퓨터 내 응용프로그램 번호
			// :9000(강의장 DB서버 이용자)
			
			String dbName = ":ORCL"; // DB이름(비슷한 단어로 SID)
			// 19c 버전 : :ORCL
			// 18c 버전 또는 강의장 DB : :XE
			...
			// 	 DB와 연결 가능한 Connection 객체를 만들어 얻어오는 역할을 하는 객체
...
//			System.out.println(conn);
			...
			// 5. Statement 객체를 통해 SQL(SELECT)을 수행하고 결과(ResultSet)를 반환 받아와서 저장(rs)
			rs = stmt.executeQuery(sql);
			// executeQuery() : SELECT문을 수행하는 메서드로서 결과로 ResultSet을 반환
			...
			while(rs.next()) {
				...
				// rs.next()
				// - ResultSet 객체는 조회 결과를 한번에 테이블로 다루는 것이 아닌 커서라는 것을 이용해서 1행씩만 접근
				// - 커서를 다음 행으로 이동 후 행이 존재하면 true, 없으면 false 반환
...
//				java.sql.Date hireDate = rs.getDate("HIRE_DATE"); -> 이것도 가능!
			...
			e.printStackTrace();
		}finally { // try에서 예외가 발생 하든말든 무조건 수행
			
			// 4단계 : 사용한 JDBC 객체 자원 반환(close())
			// -> close 구문은 객체 생성 역순으로 작성
			
			// 객체 생성 순서 : conn, stmt, rs
