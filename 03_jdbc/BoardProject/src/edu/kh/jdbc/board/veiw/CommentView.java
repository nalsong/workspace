...
public class CommentView {
	...
	private void insertComment(int boardNo) {
		   ...
		   try {
			   // 내용, 게시글 번호, 로그인 회원 번호를 이용해
	           // 댓글 삽입 서비스 호출
			   int result = service.insertComment(sb.toString(), Session.loginMember.getMemberNo()); //(제목, 내용 , 로그인회원번호)
	...
	private void updateComment(int boardNo) {
		...
		System.out.print("댓글을 수정할 게시글번호 입력:");
		boardNo = sc.nextInt();
		sc.nextLine();
		...
		if(Session.loginMember.getMemberNo() != boardNo) {
			...
			try {
				...
				//게시글 수정 서비스 호출
				int result = service.updateComment(sb.toString(), boardNo);
