...
const deleteSet = new Set(); // 순서, 중복 없음
// -> X버튼 클릭 시 순서를 한 번만 저장하는 용도 




// inputImage는 배열이기 때문에 이벤트 사용 안됨
for(let i=0; i< inputImage.length; i++) {
    ...
    inputImage[i].addEventListener('change',e=>{
        ...
        if(file != undefined){ // 파일이 선택되었을 때
            ...
            reader.readAsDataURL(file); // 지정된 파일을 읽은 후 result변수에 URL형식으로 저장

            reader.onload = e => { // 파일을 다 읽은 후 수행
                preview[i].setAttribute('src', e.target.result);

                // 이미지가 성공적으로 읽어지면 deleteSet에서 삭제
                deleteSet.delete(i);
            }
    ...
    deleteImage[i].addEventListener('click',() => {
        ...
        if(preview[i].getAttribute('src') != ""){
            ...
            inputImage[i].value=""; // // input type ="file" 의 value는 ""(빈칸)만 대입 가능
...
boardUpdateFrm.addEventListener("submit", e=>{
    ...
    // Array.from(deleteSet) : Set -> Array변경
