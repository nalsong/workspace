...
addComment.addEventListener("click", e => { // 댓글 등록 버튼이 클릭이 되었을 때
    ...
    const data = {"commentContent" : commentContent.value, 
                    "memberNo" : loginMemberNo, "boardNo" : boardNo}; // JS객체
...
function deleteComment(commentNo){
    ...
    if( confirm("정말로 삭제 하시겠습니까?") ){
        ...
        fetch("/comment",{
            method : "DELETE",
            headers : {"Content-Type": "application/json"},
            body : commentNo // 값 하나 보낼 때는 JSON필요 없음
        })
        .then(resp => resp.text())
        .then(result => {
            if(result > 0){
...
function updateComment(commentNo, btn){
    ...
    const data = {
        "commentNo" : commentNo, "commentContent" : commentContent, 
    }
...
function insertChildComment(parentNo, btn){
    ...
    const data = {"commentContent" : commentContent, 
                "memberNo" : loginMemberNo, "boardNo" : boardNo, "parentNo" : parentNo};
