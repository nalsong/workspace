...
if(secessionFrm != null){ //탈퇴 페이지인 경우
    ...
    secessionFrm.addEventListener("submit", e =>{
        ...
        if(memberPw.value.trim() ==""){ // 비밀번호 미작성 시
            alert("비밀번호를 작성해주세요");
            e.preventDefault();
            memberPw.focus();
            return;
...
if(imageInput != null){ // 화면에 imageInput이 있을 경우
    ...
    // - text/number형식 사용 가능
    // -> 이 때 input값 입력 후 포커스를 잃었을 때 이전 값과 다르면 change이벤트 발생
    imageInput.addEventListener("change", e =>{
        ...
        if(file == undefined){ 
            ...
            delectCheck = -1; // 취소는 아무것도 파일 없음(초기상태)
            profileImage.setAttribute("src", "/resources/images/user.png"); // 취소시 기존 프로필 이미지로 변경
            return;
        }
        
        if(file.size > maxSize){ // 선택된 파일의 크기가 최대 크기를 초과한 경우
            alert("2MB이하의 이미지를 선택해 주세요.");
            imageInput.value="";
            // image type="file"태그에 대입할 수 있는 value는 ""(빈칸) 뿐이다.
            delectCheck = -1; // 취소는 아무것도 파일 없음(초기상태)
            profileImage.setAttribute("src", "/resources/images/user.png"); // 취소시 기존 프로필 이미지로 변경
            return;
        }
        ...
        // 매개변수에 작성된 파일을 읽어서 저장 후 파일을 나타내는 URL을 result속성으로 얻어올 수 있게 함.
    ...
    document.getElementById("profileFrm").addEventListener("submit", e => {
        ...
        // 프로필 이미지가 없었는데 생김(기본 -> 새 프로필이미지 업로드)
        if(!initCheck && delectCheck == 1) flag = false;
        
        // 이전 프로필 이미지가 있었는데 삭제함
        if(initCheck && delectCheck == 0) flag = false;
        

        // 이전 프로필 이미지가 있었는데 새 이미지로 업로드
        if(initCheck && delectCheck == 1) flag = false;


        if(flag){ // flag == true -> 제출하면 안되는 경우
            e.preventDefault(); // from기본 이벤트 제거
