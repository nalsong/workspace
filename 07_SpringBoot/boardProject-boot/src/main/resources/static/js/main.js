...
btn3.addEventListener("click", ()=>{
    ...
    .then(memberList => {
        ...
        if(memberList.length ==0){
            result3.innerHTML="조회결과가 없습니다.";
            return;
        }


        // 향상된 for문으로 memberList순차 접근
        for(let member of memberList){

            // tr, td만들어서 result3에 추가
...
function getCookie(key) {
    ...
    // 배열.map(): 배열의 모든 요소를 순차 접근하여 함수 수행 후 수행결과를 이용해서 새로운 배열을 만드는 함수
