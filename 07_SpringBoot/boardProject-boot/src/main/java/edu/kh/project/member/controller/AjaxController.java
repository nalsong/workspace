...
public class AjaxController {
	...
	@ResponseBody
	public String selectNicname(String email) {
								// 쿼리스트링에 담긴 파라미터
...
//		return 데이터; -> 데이터를 요청한 곳으로 반환(비동기식)
	...
	public String CheckEmail(String email) {
		return service.checkEmail(email) + ""; // String으로 변환해서 반환
	}
	 ...
	 * 
	 * 2) POST : HTTP 요청 Body에 파라미터가 담겨 있으므로 @RequestBody를 통해 값(JSON)을 얻어와 
	 *           Java객체로 변환(HttpMessageConverter)
	 * 
	 * - 응답 방법(GET/POST 구분 X)
	 * : @ResponseBody를 이용해 반환
	 *   -> 해당 어노테이션을 작성하면 Controller에서 반환되는 값이 ViewResolver가 아닌 
	 *   HttpMessageConverter로 전달되어 반환된 Java객체를 text/JSON으로 변환 후 비동기 요청을 한 곳으로 응답함
	 * 
	 * 
	 * ****************************
