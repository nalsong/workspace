...
@PropertySource("classpath:/config.properties") // properties파일 내용을 현재 클래스에서 사용
public class MyPageServiceImpl implements MyPageService{
	...
	// 프로필이미지 수정 서비스
	@Override
	public int dateProfile(MultipartFile profileImage, Member loginMember) throws IllegalStateException, IOException {
		
		// 프로필 이미지 변경 실패 대비
		String temp = loginMember.getProfileImage(); // 이전 이미지 저장
		
		// 업로드 된 이미지가 있을 경우 ( <-> 없는 경우(x버튼) )
		...
		if(profileImage.getSize() > 0) { // 업로드 된 이미지가 있을 경우
			...
			loginMember.setProfileImage(webPath + rename);
										// /resources/image/member/20230510163910_12345.jpg
		}else { // 업로드 된 이미지가 없는 경우(x버튼)
			loginMember.setProfileImage(null); // 세션 이미지를 null로 변경해서 삭제
		}
		
		
		// 프로필 이미지 수정 DAO메서드 호출
