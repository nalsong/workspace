...
//		int[] arr; 
		...
		//배열 할당
		arr = new int[4];
		// - 새롭게 int형 변수 4개 짜리 배열을 생성하고 
		//   이를 arr에 대입
		//        -> arr이 해당 배열을 참고
		//        -> 해당 배열을 부를 때 arr이라고 부르면 됨.
...
//		String[] menuArr = new String[10];
		...
		String[] menuArr = {"김밥 + 라면", "서브웨이", "KFC", "맘스터치", "순대국", "뼈해장국", "닭갈비", "마라탕", "우육면", "파스타"};
	...
	public void ex5() {
		...
		for(int i=0 ; i<arr.length; i++) {  // i = 0, 1, 2, 3, 4, 5
			...
			// java.lang.Array Index OutOfBounds Exception :  Index 5 out of bounds for length 5
	...
	// 7일차(230214)
		...
		public void ex6() {
			...
			int[] scoreArr = new int[size]; //입력받은 정수의 길이를 갖는 배열 생성
			...
			int max = scoreArr[0];   // 1) 매우 작은 수를 초기화
									 // 2) 배열의 0번 인덱스 값을 초기화
...
//			toString() : 저장된 값을 하나의 문자열로 만든다.
			...
			// 깊은 복사를 진행할 arr1 배열과 같은 크기의 배열을 준비
...
//			}
			...
			// shift + F2
			// System.arraycopy(원본배열명, 원본 배열 복사 시작 인덱스, 복사 배열명, 복사 배열의 삽입 시작 인덱스, 복사 길이);
