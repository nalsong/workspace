...
public class StudentView {
	...
	public void displayMenu() {
		...
		do {
			...
			System.out.println("6. 학생 총점 점수 합계, 총점 평균, 총점 최고점, 총점 최저점 조회");
...
//	/** */ : 자바클래스나 인터페이스, 생성자, 메서드 등을 설명할 때 이용
	...
	private void selectAll() {
		...
		for(int i = 0; i<studentList.length; i++) {
			...
			// Student객체가 가지고 있는 toString()메서드를 호출해서
	...
	private void selectIndex() {
		...
		System.out.println("조회할 학생의 인덱스 번호를 입력하세요 : ");
		int index = sc.nextInt();
		
		// index에 해당하는 배열 요소를 얻어와 저장
		...
		if(std != null) {
			...
			System.out.println("해당 인덱스에 학생 정보가 존재하지 않습니다."); //NullPointerException 방지 처리
		}
	}
	...
	private void updateStudent() {
		...
		// 입력 받은 index번 째 학생의 점수를 수정하는 서비스 호출
		// = 수정 성공 시 ture/ 아니면 false
	...
	private void sumAvgMaxMin() {
		...
		System.out.println("[학생 점수 합계, 평균, 최고점, 최저점]");
