...
public class ForEx {
	 ...
	 * 
	 * 		[3][6]조건식이 true일때 반복 수행할 코드
	 * }
	 * 
	 * [1]~[4]번 수행 후 조건식 false가 나올 때까지 [5]~[7]번 반복 
	 * 
	 * 
	 *  - 초기식 : for문을 제어하는 용도의 변수를 선언 및 초기화
	 * 
	 *  - 조건식 : for문의 반복 여부를 지정하는 식
	 *  			조건식이 true인 경우에만 반복 수행함
	 *  
...
//				for(int i = 10 ; i <= 20 ; i += 0.5) {
					...
					// 왜? i += 0.5 수행지 10.5가 아닌
					// 		10 으로 ( int형으로 ) 형변환 되어 i에 대입되기 때문에 
					// 		i가 계속 10을 유지한다.
					
					//해결 방법 : 초기식을 double자료형으로 변경
				...
				//1) A(65), Z(90)의 유니코드(아스키) 번호를 이용하기
				for(int i = 65 ; i <= 90; i++) {
					
					System.out.print( (char)i ); // char 미사용시 656667687970
				}
				...
				// 3) char자료형은 문자형이지만 실제로는 정수를 저장한다.
			...
			public void ex8() {
				...
				int sum = 0; //합계를 저장할 변수 선업 및 0 으로 초기화
			...
			public void ex18() {
				...
				for(int x =1; x<=4; x++) { //4번반복 (x=1,2,3,4)
