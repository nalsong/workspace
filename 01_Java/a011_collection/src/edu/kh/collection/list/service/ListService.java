...
public class ListService {
	...
	public void ex1() {
		 ...
		 * 3) 여러타입의 객체를 저장할 수 있다.
		 * 	-> Object타입의 참조변수 묶음이기 때문에
		 * */
...
//		ArrayList list = new ArrayList(); // 기본생성자 -> 10칸짜리 생성
		...
		list.add("초과"); //List 범위3 초과
						  // -> 자동으로 크기 증가
		
		// void add(int index, E e) : 원하는 index위치에 요소 추가
		list.add(1, "중간삽입");
		
//		list.add(10, "삽입"); //오류발생
		
		// E get(int index) : 주어진 index의 요소를 반환
		//int size() : 리스트의 크기가 아니라 리스트에 저장된 요소의 개수
	...
	public void ex2() {
		...
		//String으로 타입이 제한 된 ArrayList객체를 생성하여 
		//부모 타입인 List로 참조
		
		// 컴파일 단계에서 타입체크 
		// -> 컬렉션에 사용되는 객체의 타입이 컬렉션 생성 시 지정된 타입과 같은지 확인
		// + 자동으로 다운캐스팅 진행
