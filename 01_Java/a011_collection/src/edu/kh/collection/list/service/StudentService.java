...
//	private List<Student> studentList = new ArrayList<Student>();
	...
	/**학생 주소 검색 서비스
	 * @param input
	 * @return list:List<Stuent> 검색어가 주소에 포함된 학생 리스트
	 */
	public List<Student> selectAdreess(String input) {
		
		List<Student> list = new ArrayList<Student>();
		for(Student s : studentList) {
			
			// String.contains("문자열") : String에 "문자열"이 포함되어 있으면 true반환
...
//		List<Student> list = new ArrayList<Student>();
		List<Student> list = new ArrayList<>(); 
										// 제네릭의 타입추론( <>안의 내용 생략가능 )
										// 생성되는 컬렉션 객치의 제네릭을 변도 작성하지 않아도
	...
	public List<Student> sortScore() {
		...
		// <?> void collection.sort(List<?> list)
