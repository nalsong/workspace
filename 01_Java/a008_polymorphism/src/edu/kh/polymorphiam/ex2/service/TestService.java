package edu.kh.polymorphiam.ex2.service;

import edu.kh.polymorphiam.ex2.dto.*; //dto패키지의 모든 클래스import
// * (별, 애스터리스크): 모든. ALL
...
public class TestService {
	...
	public void ex1() {
		...
		//3. 추상클래스는 객체 생성X / 부모타입 참조변수 O
		//(다형성의 업캐스팅)
		Animal a1 = new Person("포유류", "채식", "김건강", 30); 
		a1.breath(); 
		// 동적바인딩에 의해 Person에 구현된breath()가 호출됨
		
		
		
		// 객체 배열 + 추상클래스
		Animal[] arr = new Animal[3];
		// Animal 객체 참조변수의 묶음을 생성
		// (Animal 객체는 만들 수 없다!)
	...
	public void ex2() {
		...
		// - 인터페이스에 작성했지만 클래스에 작성하는 static필드와 사용법이 동일
		// - 인터페이스 필드는 묵시적(암묵적)으로 public static final
		
		System.out.println(KH.KH_ADERSS);
...
//		KH kh = new KH() //객체 생성X
		...
		//객체 배열 + 다형성 + 동적 바인딩
		KH[] arr = new KH[2]; //KH참조 변수의 묶음 생성 
	...
	public void ex3() {
		...
		Calculator cal = new JSYCalculator();
