...
public class ExceptionService {
	...
	public void ex1() throws IOException {
		...
		// 왜 빨간줄이 뜰까? 
		// - readLine()메서드는 IOException이라고 하는 예외를
		// 발생시킬(던질) 가능성이 있기 때문에 그 상황에 대한 대비책(예외처리)를 하라고 경고하는 것
		...
		// -> 공식 API 문서의 메서드 설명에 throws OOOException으로작성 되어있는 메서드가 있으면 
		// 해당 코드 사용 시 문제가 발생할 것이다라고 생각하고 그 상황에 대한 예외 처리 코드를 꼭 작성
	...
	public void ex2() {
		...
		try{
			...
			//Checked Exception이 발생할 경우를 대비해서 예외처리 구문 작성
...
//		catch(Exception e) {}  @@
			...
			//!!!!!!!!주의사항!!!!!!!!!!!!
			//- 상위 타입을 처리하는 catch문을 하위 타입을 처리하는 catch문 보다 먼저 작성하면 오류 발생
...
//			e.getMessage();
	...
	public void methodB() throws IOException{
		...
		methodC();
		//methodC는 IOException을 던질 수도 있기 때문에 호출시 예외 처리 구문을 작성해야 한다.
	}
	...
	public void ex5() throws ScoreInputException{
		//사용자 정의 예외
		// - Java에서 제공하지 않는 예외 상황이 있을 경우 이를 처리하기 위한 예외 클래스를 사용자가 직접 작성
