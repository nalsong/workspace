...
public class APIView {
	...
	public void displayMenu() {
		...
		do {
			...
			try {
				...
				sc.nextLine(); // 입력버퍼에 남아있는 잘못 입력된 문자열을 읽어와 없앰(무한루프 종료)
	...
	private void ex1() {
		...
		sc.nextLine(); // 입력버퍼에 남아있는 개행문자를 제거
	...
	public void ex2() {
		...
		// 이름을 여러명 한 줄로 입력 받아 학생 배열에 같은 이름의 학생이 있다면 출력
		System.out.println("\n === 학생검색 ===\n");
	...
	public void ex4() {
		...
		// 1. String 객체 생성 방법
		String s1 = new String("abc"); // Heap 메모리 영역에 String객체가 생성
		
		String s2 = "abc"; // String은 사용 빈도가 높기 때문에 별도의 리터럴표기법을 부여하여 쉽게 객체 생성
...
//		StringBuffer sb = new StringBuffer();
		...
		// String의 불변성으로 인해 객체는 변하지만 주소는 변하지 않음!
		
		//새로운 문자열이 추가되어도 객체의 주소는 변하지 않음== 가변성(mutable)
...
//			sb.append(String str) : 기존 문자열 뒤에 추가
	...
	private void ex7() {
		...
		/*Wrapper Class
		 * - 기본자료형을 객체로 다룰 수 있도록 포장하는 클래스
		 * 
		 *  왜 포장이 필요한가?
		 *  1) 기본자료형이 제공하지 않는 추가 필드, 메서드 제공하기 위해
		 *  2) 기본자료형을 객체로 다뤄야 하는 경우가 있기 때문에
		 *  	(컬렉션)
		 * */ 
		...
		System.out.println(Integer.TYPE); //Wrapper클래스 대상 타입
		...
		String str3 = Double.valueOf(999.99).toString();
		String str2 = 999 + "";	// "" : 빈 문자열(내용이 없는 길이 0짜리 String)	
		 ...
		 * UnBoxing : Wrapper Class 객체 -> 기본자료형
		 * 
		 * AutoBoxing / AutoUnBoxing
		 * - 사용자가 신경쓰지 않아도 상황에 따라 
		 * 기본자료형 <-> Wrapper Class 객체로 자동으로 변하는 기술
		 * */
	...
	private void ex8() {
		...
		// -> 기준 시간으로부터 지난 시간(ms)을 나타냄 
		System.out.println("현재시간 - 기준시간(ms) : " + System.currentTimeMillis());
		
		// 현재 시간 부터 1시간 후를 저장
		long temp = 60 * 60 * 1000; //3600000ms == 1시간
		// 			분   초    ms
