...
<body>
    ...
    <a href="condition">JSTL을 이용한 조건문(if, choose, when, atherwise)</a>
    ...
    <pre>
        JSP에서 자주 사용되거나 공통적으로 사용되는 JAVA코드(if, for, 변수선언, 형변환)를 

        스크립틀립 대신 html 태그 형식을 태그화하여 표준으로 제공하는 라이브러리
        (if, for 간단히 쓰고 싶으면 이 라이브러리를 사용해라.)
    </pre>

    <h3>JSTL 라이브러리 다운로드 및 등록방법</h3>

    <ol>
        <li><a href="https://tomcat.apache.org/download-taglibs.cgi">JSTL 다운로드 페이지 이동</a>
            <br>
            Download-> Taglibs -> Standard-12.5 -> Jar Files -> Impl, Spac, EL 다운로드
        </li>
        <li>
            /wevapp/WEB-INF/lib 폴더에 라이브러리 파일(.jar)추가(이클립스에서 복붙)
        </li>
        <li>
            JSTL 라이브러리를 사용하고자 하는 JSP파일 상단에 taglib JSP지시자 태그를 추가(2번줄에 작성된 내용)
        </li>

    </ol>

	<hr>
    <h1>1. 변수선언 (c:set태그)</h1>
    ...
    <pre>
        ...
        1) var : 변수명(속성key)
        2) value : 대입할 값
        3) scope : page, request, session, application 중 하나 지정(기본값은 page)
    </pre>

    <%
        // 스트립틀릿으로 page scope에 속성 세팅하는 방법
    ...
    <br>
    page request num2 제거 확인 : ${pageScope.num2}

    <hr>

    <h1>3. 변수 출력 (c:out태그)</h1>

    <pre>
        -변수 출력 : \${key} EL 구문과 비슷함

        - 단, escapeXmml="true"(기본값) 설정 시 html태그가 해석X
        - escapeXmml="false" : html태그 해석 O
    </pre>
