...
// calc.addEventListener("click", () => {  // 내가 푼 것
document.getElementById("calc").addEventListener("click", () => { // 강사님 풀이
...
document.getElementById("add").addEventListener("click", () => {
    ...
    // 요소에 class 추가 : 요소.classList.remove("클래스명")

    // 요소에 속성, 값 추가 : 요소.setAttribute("속성명", 값);
    // 요소에 속성제거      : 요소.removeAttribute("속성명");

    // 부모요소.append(자식요소) : 부모요소의 마지막 자식으로 자식요소를 추가(덧붙이기)
    // 부모요소.prepend(자식요소) : 부모요소의 첫 번째 자식으로 자식요소를 추가
    ...
    // span 에 &times; 내용 추가(innerHTML사용)
    span.innerHTML="&times;";


    // 만들어진 span요소에 이벤트리스너 추가
    span.addEventListener("click", (e) => {
        // 클릭된 요소의 부모 요소를 삭제 == row삭제
