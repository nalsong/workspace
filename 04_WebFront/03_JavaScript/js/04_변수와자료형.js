...
// 전역 변수
var str = "JS코드를 함수 내부가 아닌 JS파일 또는 script태그에 바로 작성하면 HTML랜더링 시 바로 수행된다.";
...
// num3 is not defined( 정의되지 않음 == 존재하지 않거나 접근 불가능)
// console.log("함수 밖에서 (함)num3 : " + num3);

// JS코드가 중간에 에러가 발생하면 같은 파일/함수 내 에러 이후 부분 코드가 해석되지 않는다.
...
function typeTest(){
    ...
    console.log(user); // 콘솔로 객체 출력 시 브라우저가 파악하기 쉽게 바꿔서 출력
