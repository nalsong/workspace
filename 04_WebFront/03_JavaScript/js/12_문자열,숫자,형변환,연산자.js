...
document.querySelector('#btn1').addEventListener('click', () => {
    ...
    const str2 = "돈까스,김치찌개,샌드위치,햄버거,떡볶이,김밥,마라탕,초밥";
...
document.querySelector("#btn2").addEventListener("click", function(e){
    ...
    // isNaN(연산/값) : 연산/값의 결과가 NaN이면 true반환
    if(isNaN("abc"*100)){
    ...
    // 부동소수점(부정확, 용량 작음) -> 고정 소수점(정확, 용량 큼) 변경

    // 숫자.toFixed(보여질 소수 자릿수)
    // - 주어진 숫자를 매개변수에 작성된 숫자 만큼의 소숫적 자릿수로 표현될 수 있도록
    ...
    // Math.random() -> 0.0 <= x < 1.0 실수반환
    
    // 버튼 클릭 시 마다 버튼 배경색 랜덤하게 변경

    // 0~255
    const r = Math.floor(Math.random() * 256); // 0.0 <= x < 266
    const g = Math.floor(Math.random() * 256); // 0.0 <= x < 266
    const b = Math.floor(Math.random() * 256); // 0.0 <= x < 266
