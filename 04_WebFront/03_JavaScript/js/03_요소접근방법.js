...
function accessId(){ // 함수 선언
    ...
    // 2) div1의 배경색을 변수에 저장
    /* JS에서 CSS관련 속성을 다룰 때는 style을 이용함 */
    var bgColor = div1.style.backgroundColor;

    // 3) 배경색이 red일 때 , 아닐 때에 대한 코드 작성
    if(bgColor == "red"){ //JS는 문자열 비교시에도 비교연산자 사용
...
function accessName(){
    ...
    for(var i = 0; i < hobbyList.length; i++){
        ...
        // 해당 요소가 체크되어 있으면 true, 아니면 false반환

        if(hobbyList[i].checked){ // 체크되어 있을 경우
            result +=hobbyList[i].value + " "; // value값 누적
            count++; // 1증가
        }
    }
    // 4) #div3에 내용 출력
...
function cssTest(){
    ...
    // querySelectorAll("선택자") : 일치하는 모든 요소를 선택(배열 반환)
...
function readValue(){
    ...
    if(input.value.trim().length > 0){
        ...
        // 채팅 내용 출력 후 container의 스크롤을 제일 밑으로 내리기
