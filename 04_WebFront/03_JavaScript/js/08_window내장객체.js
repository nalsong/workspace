...
document.getElementById("testBtn").addEventListener("click", () => {
    ...
    // setTimeout같은 시간 관련 함수들은 여러 번 작성된 경우 앞에 코드가 실행되길 기다리지 않음.
    // -> 비동기로 동작
});
...
function currentTime() {
    const now = new Date(); // 현재시간을 저장한 Date객체 생성
    ...
    // 시/ 분 /초 가 10 미만 ( 두자리가 아닐 경우)인 경우 앞에 0 추가
...
// 처음에 화면에 시간이 표시되도록 currentTime호출
currentTime();
...
openPopup3.addEventListener("click", () => {
    ...
    // 열리는 위치는 위에서 100px, 왼쪽에서 100px떨어진 위치에서 열기
    // window.open("url", "popup");
...
document.getElementById("sendPopup").addEventListener("click", () => {
    window.open("09_팝업.html", "popupWindow", "width=500, height=500");
});
