...
// == 스프링이 해당 클래스를 객체로 만들어서 관리를 해야 한다.
// == Bean등록

@Component // @Controller, @Service, @Repository의 부모 어노테이션
			// Bean읃록을 하겠다고 명시하는 어노테이션
...
//		System.out.println("스케쥴러가 일정 시간마다 자동으로 출력");
		...
		// 서버에 저장된 파일 목록을 조회해서 DB에 저장된 파일 목록과 비교하여 매칭되지 않는 서버파일 제거
		// 1) 서버에 저장된 파일 목록 조회
		// -> application객체를 이용해서 /resoutces/images/board의 실제 서버 경로를 얻어옴
