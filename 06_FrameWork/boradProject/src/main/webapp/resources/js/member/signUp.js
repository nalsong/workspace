...
정규표현식 (Regular Expression)
    ...
    - 문자열에 대한 검색, 일치여부, 치환 등을 수행할 수 있음
    ...
    *** 정규표현식 객체가 제공하는 메서드(함수) ***
    ...
    2. regEx.exec(문자열)
        -> 문자열이 정규표현식 패턴에 부합하면 첫 번째 매칭되는 문자열을 반환
            없으면 null반환
     ...
     *** 정규 표현식의 메타 문자***
        ...
        [0-9]  : 0부터 9까지 모든 숫자
        [ㄱ-힣] : ㄱ부터 힣까지  모든 한글
        [가-힣] : 가부터 힣까지  모든 한글(자음 또는 모음만 있는 경우는 제외)
        [a-z] : 모든 영어 소문자
        [A-Z] : 모든 영어 대문자
...
// JS 객체 : { "Key":Value, "K":V, "K":V, "K":V,... } (Map형식)
// 특징
...
// 3) 객체에 특정 Key를 삭제할 수 있다. (delete연산자)
// ex) const obj = {"a":1, "b":2}
//      delete obj.b; // {"a":1}

/* 유효성 검사 진행 여부 확인용 객체 */
// -> 모든 value가 ture인 경우에만 회원 가입 진행 
const checkObj = {
    "memberEmail" : false, 
    "memberPw" : false,
...
memberEmail.addEventListener("input", () => {
    ...
    if(memberEmail.value.trim().length == 0) {
        ...
        // confirm, error클래스 삭제해서 검정글씨로 만들기
...
memberPw.addEventListener("input", () => {
    ...
    if(regEx.test(memberPw.value)){
        ...
        if(memberPwConfirm.value.trim().length == 0){ // 비밀번호가 유효하게 작성된 상태에서 비밀번호 확인이 입력되지 않았을 때
...
document.getElementById("signUpFrm").addEventListener("submit", e=>{
    ...
    // (객체용 for문)
    // ** for ... in구문 ***
    // -> JS객체가 가지고 있는 Key를 순서대로 하나씩 꺼내는 반복문
...
memberNickname.addEventListener("input", () => {
    ...
    // 닉네임에 입력이 되지 않은 경우
    if(memberNickname.value.trim().length == "") {
...
memberTel.addEventListener("input", ()=>{
    ...
    if(regEx.test(memberTel.value)){ //유효할 때
