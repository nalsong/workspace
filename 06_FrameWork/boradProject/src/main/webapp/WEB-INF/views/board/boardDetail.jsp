...
<body>
            ...
            <!-- 이미지가 있을 경우 -->
                <c:if test="${not empty board.imageList}" >
                    ...
                    <%-- 
                        ...
                        -> imageList에 썸네일이 있다면 조회되었을 때 IMG_ORDER가 0인 이미지가 imageList[0]에 저장되었을 것이다.
                    --%>
    ...
    <%-- 
        누가(로그인한 회원 번호) 어떤 게시글(현재 게시글 번호)에 좋아요를 클릭/취소 했는지 확인하기 

        로그인한 회원번호 얻어오는 방법
        1) ajax로 session에 있는 loginMember의 memberNo를 반환
        2) HTML요소에 로그인한 회원의 번호를 숨겨놓고 JS로 얻어오기 
        3) jsp파일 제일 위에 있는 script태그에 JS + EL 이용해서 전역 변수로 선언해두기

    --%>

    <script>
        // JSP에서 작성 가능 한 언어/라이브러리 : html, css, js, java, EL, JSTL

        // JSP해석 우선 순위 : Java/EL/JSTL -> HTML, CSS, JavaScript

        // 게시글 번호 전역 변수로 선언
        const boardNo = ${board.boardNo};

        // 로그인한 회원 번호를 전역 변수로 선언
        // -> 작성한 EL구문이 null일 경우 빈칸으로 출력되어 변수에 값이 대입되지 않는 문제가 발생할 수 있음!
        // 해결방법 : EL구문을 '', ""로 감싸 문자열로 만들면 해결
        //          -> EL값이 null이여도 ""(빈문자열)로 출력
