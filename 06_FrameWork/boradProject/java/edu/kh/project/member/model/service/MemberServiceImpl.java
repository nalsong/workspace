...
@Service // Service Layer(서비스 클래스)
		// 비즈니스 로직(데이터 가공, DAO호출, 트랜젝션 제어)처리하는 클래스라 명시
		// + Bean 등록하는 어노테이션
public class MemberServiceImpl implements MemberService{
	
	
	// @Autowired : 작성된 필드와 Bean으로 등록된 객체 중 타입이 일치하는 Bean을 해당 필드에 자동 주입(Injection)하는 어노체이션
...
//		System.out.println("암호화 확인 : " + bcrypt.encode(inputMember.getMemberPw()));
	...
	// -> 예외가 발생하면 rollback 발생 안하면 Service종료 시 commit
	...
	public int signUp(Member inputMember) {
		...
		// 비밀번호를 SCrypt를 이용하여 암호화 후 다시 inputMember 에 세팅
