...
public class MyPageServiceImpl implements MyPageService{
	...
	public int updateInfo(Member updateMember) {
	 ...
	 // 비밀번호 변경 서비스
	//@Transactional //Unchecked Exception발생 시 rollback
	@Transactional(rollbackFor = Exception.class) //모든 Exception발생 시 rollback
	@Override
	public int change(String currentPw, String newPw, int memberNo) {
		
		// 1. 현재 비밀번호 == DB에 저장된 비밀번호 비교
		
		// 1) 회원번호가 일치하는 MEMBER테이블 행의 MEMBER_PW조회
		String encPw = dao.selectEncPw(memberNo); 
		
		// 2) bcrypt.matches(평문, 암호문) -> 같으면 true ->  이때 비번 수정
		if(bcrypt.matches(currentPw, encPw)) {
			// 2. 비밀번호 변경(UPDATE DAO호출) -> 결과반환
