...
@Service // 비즈니스 로직 처리 클래스 + Bean등록 (IOC)
public class MyPageServiceImpl implements MyPageService{

	@Autowired // MyPageDAO를 의존성 주입(DI)
	...
	// 회원정보 수정 서비스
	@Transactional(rollbackFor = {Exception.class}) // 외워서 쓰면 됨.
	@Override
	...
	//@Transactional //Unchecked Exceprion발생 시 rollback
	@Transactional(rollbackFor = Exception.class) //모든 Exceprion발생 시 rollback
	@Override
	public int changePw(String currentPw, String newPw, int memberNo) {
		// 1.현재 비밀번호, DB에 저장된 비밀번호 비교
		// 1) 회원 번호가 일치하는 MEMBER테이블 행의 MEMBER_PW조회
		String encPw = dao.selectEncPw(memberNo);
		
		// 2) bcrypt.matches(평문, 암호문)-> 같으면true -> 이 때 비번 수정
		if(bcrypt.matches(currentPw, encPw)) {
			
			// 2. 비밀번호 변경(UPDATE DAO 호출) -> 결과반환
