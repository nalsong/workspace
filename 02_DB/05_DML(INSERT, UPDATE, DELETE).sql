...
UPDATE EMPLOYEE2
SET BONUS = 0.1
WHERE BONUS IS NULL; --행 수정

SELECT * FROM EMPLOYEE2; -- 수정확인
...
SELECT * FROM DEPARTMENT2;
--> 조건을 설정하지 않으면 지정된 테이블의 모든 해잉 수정된다!!
--> WHERE절 작성을 잘하자
...
UPDATE EMPLOYEE2
SET (SALARY, BONUS) = (SELECT SALARY, BONUS   -->괄호 안의 내용이 다중열 서브쿼리
                       FROM EMPLOYEE2 
                       WHERE EMP_NAME = '유재식')
WHERE EMP_NAME IN ('노옹철', '전형돈', '정중하', '하동운');
...
SELECT * FROM EMPLOYEE2 WHERE SALARY > 3000000; --SELECT * 위치에 DELETE를 넣으면 됨
DELETE FROM EMPLOYEE2 WHERE SALARY > 3000000;
...
-- 5. TRUNCATE (DDL 입니다! DML 아닙니다!)  --> 잘라버리기
-- 테이블의 전체 행을 삭제하는 DDL
