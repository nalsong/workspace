...
--	(EXTRACT(YEAR FROM SYSDATE) - EXTRACT(YEAR FROM TO_DATE(SUBSTR(EMP_NO, 1,INSTR(EMP_NO, '-')-1)))) "만 나이"
--	TRUNC((SYSDATE - TO_DATE(SUBSTR(EMP_NO, 1,INSTR(EMP_NO, '-')-1)))/365)"만 나이"
	FLOOR(MONTHS_BETWEEN(SYSDATE,TO_DATE(SUBSTR(EMP_NO, 1,INSTR(EMP_NO, '-')-1)))/12) "만 나이"
	
 --	FLOOR((SYSDATE-TO_DATE(SUBSTR(EMP_NO, 1, 6)))/10000) "왜이래",

 --TO_CHAR(SUBSTR(EMP_NO, 1, 6)+1900, 'YYYYMMDD')
 
--	(TO_CHAR(SYSDATE, 'YYYYMMDD')-TO_CHAR(SUBSTR(EMP_NO, 1, 6), 'YYYYMMDD'))/10000
