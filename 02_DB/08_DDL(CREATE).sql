...
CREATE TABLE MEMBER(
	...
	MEMBER_NAME VARCHAR2(15), --한글(3BYTE) 2글자 ~ 5글자
	MEMBER_SSN CHAR(14),	 -- 990123-1234567 (항상 14글자)
	ENROLL_DATE DATE DEFAULT SYSDATE -- 가입일
);

-- DEFAULT(기본값) : INSERT/UPDATE 시 해당 칼럼의 값을 작성하지 않거나 
-- 					DEFAULT로 작성하면 DEFAULT 오른쪽에 작성된 값의 컬럼에 반영됨


-- 만든 테이블 확인
SELECT * FROM MEMBER;
SELECT * FROM USER_TABLES;
				-- USER_TABLES: 딕셔너리 뷰(데이터 딕셔너리에서 일부만 뽑아 보여주는 가상 테이블)
...
INSERT INTO MEMBER 
VALUES ('MEM03', '1Q2W3E4R', '박철수', '950505-1987654', DEFAULT);
...
COMMIT; --TCL -> 트랜잭션에 저장된 DML구문을 DB에 반영

ROLLBACK; -- 마지막 COMMIT상태로 돌아감(트랜잭션에 저장된 DML 삭제)
...
INSERT INTO MEMBER2 VALUES ('고길동', 9001011234567, 01012341234);
INSERT INTO MEMBER2 VALUES ('김길동', 0001013234567, 01012341234);



SELECT * FROM MEMBER2;
-- 9001011234567	1012341234
-- 1013234567	    1012341234

--> NUMBER타입의 컬럼은 값 제일 앞 0을 유지하지 않는 문제 발생
--> 문제 해결 방법 : CHAR, VARCHAR2타입으로 컬럼 타입을 지정
...
-- ORA-01400: NULL을 ("KH_JSY"."USER_USED_NN"."USER_NO") 안에 삽입할 수 없습니다
...
CREATE TABLE USER_USED_UK(
    ...
    --테이블 레벨 제약조건 설정
    , CONSTRAINT USER_ID_UK UNIQUE(USER_ID) --제약조건(컬럼명) (이름 o)
);
...
-- 방법1(이름 미지정) 테이블에서 실행 시 
-- ORA-00001: 무결성 제약 조건(KH_JSY.SYS_C007460)에 위배됩니다
-- 오류에는 UNIQUE 제약 조건이라는 말이 안 나옴

-- 방법2(이름 o) 테이블에서 실행 시
-- ORA-00001: 무결성 제약 조건(KH_JSY.USER_ID_UK)에 위배됩니다
-- 오류에 이름 표기 됨!
...
INSERT INTO USER_USED_UK2
VALUES(1, 'user01', 'pass01', '홍길동', '남', '010-1234-5678', 'hong123@kh.or.kr');
-- ORA-00001: 무결성 제약 조건(KH_JSY.USER_ID_NAME_UK)에 위배됩니다
...
-- NOT NULL + UNIQUE 제약조건의 의미( 의미만 같음)
...
CREATE TABLE USER_USED_PK(
    USER_NO NUMBER PRIMARY KEY, -- 컬럼레벨(제약조건명X)
...
--2) 컬럼레벨(제약조건명O)
CREATE TABLE USER_USED_PK(
    USER_NO NUMBER CONSTRAINT USER_NO_PK PRIMARY KEY, -- 컬럼레벨(제약조건명O)
...
--> 기본키 중복으로 오류
-- ORA-00001: 무결성 제약 조건(KH_JSY.USER_NO_PK)에 위배됩니다

INSERT INTO USER_USED_PK
VALUES(NULL, 'user03', 'pass03', '유관순', '여', '010-9999-3131', 'yoo123@kh.or.kr');
--> 기본키가 NULL 이므로 오류
-- ORA-01400: NULL을 ("KH_JSY"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
...
-- 회원 번호와 아이디 둘 다 중복 되었을 때만 제약조건 위배 에러 발생
-- ORA-00001: 무결성 제약 조건(KH_JSY.PK_USERNO_USERID)에 위배됩니다
...
-- ORA-01400: NULL을 ("KH_JSY"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다
--> 복합키로 PK가 설정되어 있어도 결국은 PK ==> NULL 삽입 불가능!
...
--> NULL(외부테이블에 참조할 값이 없다, 참조X) 사용 가능.  
...
-- ORA-02291: 무결성 제약조건(KH_JSY.GRADE_CODE_FK1)이 위배되었습니다- 부모 키가 없습니다
-- GRADE_CODE 가 부모, USER_USED_FK가 자식
...
WHERE GRADE_CODE = 20;
--> GRADE_CODE = 20번 행은 자식테이블에서 참조하고 있지 않으므로 삭제가 제한되지 않음!!


-- 참조를 하고 있는 GRADE_CODE 10을 삭제
DELETE FROM USER_GRADE
WHERE GRADE_CODE = 10;
-- ORA-02292: 무결성 제약조건(KH_JSY.GRADE_CODE_FK1)이 위배되었습니다- 자식 레코드가 발견되었습니다
-- 레코드 : 기록

SELECT * FROM USER_USED_FK; -- 자식테이블
...
CREATE TABLE USER_USED_CHECK(
  ...
  -- 컬럼 레벨
  GENDER VARCHAR2(10) CONSTRAINT GENDER_CHECK CHECK (GENDER IN ('남', '여')), -- 남, 여
...
-- ORA-02290: 체크 제약조건(KH_JSY.GENDER_CHECK)이 위배되었습니다.

INSERT INTO USER_USED_CHECK
VALUES(2, 'user02', 'pass02', '홍길동', '여', '010-1234-5678', 'hong123@kh.or.kr');

INSERT INTO USER_USED_CHECK
VALUES(2, 'user02', 'pass02', '홍길동', '여자', '010-1234-5678', 'hong123@kh.or.kr');
-- GENDER 컬럼에 CHECK 제약조건으로 '남' 또는 '여'만 기록 가능한데 '여자'라는 조건 이외의 값이 들어와 에러 발생
-- ORA-02290: 체크 제약조건(KH_JSY.GENDER_CHECK)이 위배되었습니다.
...
CREATE TABLE USER_TEST(
	...
	PNO VARCHAR(14) CONSTRAINT NN_PNO NOT NULL ,
	GENDER VARCHAR(2) CONSTRAINT CK_GENDER (CHECK IN(GENDER, '남', '여')),
	PHONE VARCHAR(10),
	ADDRESS VARCHAR(20),
	STATUS VARCHAR(2) CONSTRAINT CK_STATUS (CHECK IN(STATUS, 'Y', 'N'))
	,CONSTRAINT UK_PNO UNIQUE(PNO)
	,CONSTRAINT NN_STATUS NOT NULL (STATUS)
);
...
-- AS 서브쿼리;  -> 서브쿼리의 걸과(RESULT SET)의 형태로 테이블 생성
...
--ALTER TABLE 테이블명 ADD [CONSTRAINT 제약조건명] 
--  FOREIGN KEY(컬럼명) REFERENCES 참조 테이블명(참조컬럼명)
ALTER TABLE EMPLOYEE ADD CONSTRAINT KF_DEPT_CODE 
