...
-- 7. 직급별 나이가 가장 어린 직원의
-- 사번, 이름, 직급명, 만 나이, 보너스 포함 연봉을 조회하고
-- 만 나이순으로 내림차순 정렬하세요
-- 단 연봉은 \124,800,000 으로 출력되게 하세요. (\ : 원 단위 기호)


SELECT EMP_ID, EMP_NAME, JOB_NAME, 
	FLOOR(MONTHS_BETWEEN(SYSDATE, SUBSTR(EMP_NO, 1, 6)) /12) "만나이", 
	TO_CHAR(SALARY + (SALARY * NVL(BONUS,0))*12, 'L999,999,999') "보너스 포함 연봉"
FROM EMPLOYEE MAIN
LEFT JOIN JOB USING JOB_CODE
WHERE FLOOR(MONTHS_BETWEEN(SYSDATE, SUBSTR(EMP_NO, 1, 6)) /12) 
IN (SELECT MIN(FLOOR(MONTHS_BETWEEN(SYSDATE, SUBSTR(EMP_NO, 1, 6)) /12))
				FROM EMPLOYEE SUB
				GROUP BY JOB_CODE
