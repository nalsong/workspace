...
/*  서브쿼리 유형
    ...
    - 다중열 서브쿼리 : 서브쿼리의 SELECT 절에 나열된 항목수가 여러개 일 때 (행1, 열N)
...
NATURAL JOIN JOB
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
WHERE DEPT_TITLE = (SELECT DEPT_TITLE, JOB_NAME 
					FROM EMPLOYEE 
					NATURAL JOIN JOB
					JOIN DEPARTMENT d ON (DEPT_CODE = DEPT_ID)
					WHERE ENT_YN = 'Y'
...
-- 본인 직급의 평균 급여를 받지 않고 있는 직원
	SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY 
FROM EMPLOYEE 								
WHERE (JOB_CODE, SALARY) NOT IN(SELECT JOB_CODE, TRUNC(AVG(SALARY), -4)
							FROM EMPLOYEE
							GROUP BY JOB_CODE);						

						
-- ANY는 사용 불가 ( 대소비교불가하기 때문)
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY 
FROM EMPLOYEE 								
WHERE (JOB_CODE, SALARY) ANY (SELECT JOB_CODE, TRUNC(AVG(SALARY), -4)
							FROM EMPLOYEE
							GROUP BY JOB_CODE);							
...
-- 이태림이 포함되어 있어 틀림 
SELECT EMP_NAME, EMP_NAME , DEPT_TITLE, MANAGER_ID 
FROM EMPLOYEE
LEFT JOIN DEPARTMENT  ON (DEPT_CODE = DEPT_ID)
WHERE MANAGER_ID IS NOT NULL;
...
JOIN JOB USING (JOB_CODE)
WHERE HIRE_DATE = (SELECT MIN(HIRE_DATE) FROM EMPLOYEE SUB
					WHERE (ENT_YN ='N' AND NVL(SUB.DEPT_CODE, '1') = NVL(MAIN.DEPT_CODE, '1')) --NULL x인 경우
					)
ORDER BY HIRE_DATE;
...
-- 선동일 송종기 정중하 대북혼 노홍철 이 나와야 함!

--문제 원인 : SELECT 해석 순서상 ORDER BY가 가장 마지막으로 해석되기때문에
...
FROM EMPLOYEE 
ORDER BY SALARY DESC;
--> 해결 1의 조회 결과(RESULT SET)를 가상의 테이블(VIEW)로 이용할 예정(행 순서, 컬럼 그대로 이용)


-- 해결 2) 해결 1의 조회결과를 FROM절에 작성하여 상위 5행만 조회
...
SELECT * FROM( SELECT DEPT_CODE, DEPT_TITLE, FLOOR(AVG(SALARY)) 평균
				FROM EMPLOYEE
				LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
...
-- **인라인뷰 사용 시 컬럼명에 유의해야 한다!!**
	--> 메인쿼리에서 인라인 뷰릐 컬럼명을 그대로 사용해야 한다!
 -- 급여 평균이 590만 인 부서를 조회
SELECT * FROM(
...
-- WITH를 이용해서 MAIN쿼리를 깔끔하게 정리
 WITH TOP_SALARY AS (SELECT EMP_NAME, SALARY FROM EMPLOYEE ORDER BY SALARY DESC)
