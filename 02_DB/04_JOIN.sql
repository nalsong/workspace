...
-- ANSI
-- 
SELECT EMP_ID , EMP_NAME , DEPT_CODE , DEPT_ID, DEPT_TITLE -- DEPT_CODE , DEPT_ID 가 같은 값을 저장하고 있으나 컬럼명이 다름!
FROM EMPLOYEE 
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID);
...
SELECT EMP_ID, EMP_NAME, JOB_CODE, JOB_NAME
FROM EMPLOYEE, JOB
WHERE JOB_CODE = JOB_NAME;
-- SQL Error [918] [42000]: ORA-00918: 열의 정의가 애매합니다

-- 테이블이름 사용해서 해결
SELECT EMP_ID, EMPLOYEE.EMP_NAME, JOB.JOB_CODE, JOB_NAME
FROM EMPLOYEE, JOB
WHERE EMPLOYEE.JOB_CODE = JOB.JOB_NAME;

-- 별칭 사용해서 해결
SELECT EMP_ID, EMP_NAME, E.JOB_CODE, JOB_NAME
FROM EMPLOYEE E, JOB J
WHERE E.JOB_CODE = J.JOB_NAME;
...
FROM EMPLOYEE
/*INNER*/ JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID); --INNER는 기본값
...
-- 기존 INNER JOIN결과에 JOIN구분 왼쪽에 작성된 EMPLOYEE 테이블의 행 중에서 
-- JOIN결과에 포함되지 않은 행들도 조회하도록 함
...
-- 오라클 구문
SELECT EMP_NAME, DEPT_TITLE, DEPT_ID, DEPT_TITLE
FROM EMPLOYEE, DEPARTMENT
WHERE DEPT_CODE(+) = DEPT_ID;
...
-- 같은 테이블을 조인.
-- 자기 자신과 조인을 맺음
--> 하나의 테이블을 가지고 생각하지 말고 같은 테이블이 2개가 있다고 생각하면 쉬움


--EMPLOYEE테이블에서 사번, 이름, 사수사번, 사수이름 조회
...
FROM EMPLOYEE EMP
JOIN EMPLOYEE MGR ON (EMP.MANAGER_ID = MGR.EMP_ID);-- INNER JOIN 방식이라 MANAGER_ID에 없는 사람 제외



-- LEFT JOIN 하면 MANAGER_ID에 없는 사람 포함가능
SELECT EMP.EMP_ID, EMP.EMP_NAME, EMP.MANAGER_ID, MGR.EMP_NAME
FROM EMPLOYEE EMP
...
SELECT EMP.EMP_ID, EMP.EMP_NAME, 
	NVL(EMP.MANAGER_ID,'없음'), NVL(MGR.EMP_NAME, '없음') -- NULL 값 '없음'으로 처리
...
--NATURAL JOIN이 성립되지 않는 경우 -> CROSS JOIN의 결과가 조회됨
-- 같은 컬럼이 없으면 성립되지 않음
...
-- *JOIN순서 중요!!!*
--> 앞선 두 테이블(EMPLOYEE, DEPARTMENT)의 조인 후 조인된 결과에 LOCATION테이블을 조인
...
AND LOCATION_ID = LOCAL_CODE
WHERE JOB_NAME = '대리' -- WHERE이 두개라서 오류남
AND LOCAL_NAME LIKE 'ASIA%';
...
--SELECT EMP_NAME, JOB_CODE, SALARY, (SALARY*(1+BONUS))*12 "연봉"
--FROM EMPLOYEE
--JOIN JOB USING (JOBCODE);
-- WHERE SALARY > MIN_SAL 




SELECT EMP_NAME, JOB_CODE, SALARY, (SALARY*(1+NVL(BONUS,0)))*12 연봉
FROM EMPLOYEE
NATURAL JOIN JOB
...
SELECT EMP_NAME 사원명, DEPT_TITLE 부서명, LOCAL_NAME 지역명, DECODE(NATINAL_CODE,'KO', '한국', 'JP', '일본') 국가명 
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
WHERE DECODE(NATINAL_CODE,'KO', '한국', 'JP', '일본')
ORDER BY EMP_ID;



-- 8. 같은 부서에 근무하는 직원들의 사원명, 부서코드, 동료이름(같은부서 사람)을 조회하시오.
-- SELF JOIN 사용
-- 동료 == 같은 부서 == 같은 부서코드
...
-- 강사님 풀이 3(USING, EMP_ID)
SELECT E1.EMP_NAME 사원명, DEPT_CODE 부서코드, E2.EMP_NAME 동료이름
FROM EMPLOYEE E1
